...
def test_is_job_matches_and_mismatches(input_id, expected):
    ...
    assert informer.matches_job(input_id) == expected
...
def test_informer_from_file_with_host(tmp_path):
    ...
    with patch(
        "qq_lib.info.informer.Info.from_file", return_value=qqinfo_mock
    ) as mock_from_file:
        informer = Informer.from_file(dummy_file, host="remote_host")
...
def test_informer_from_file_no_host(tmp_path):
    ...
    with patch(
        ...
        informer = Informer.from_file(dummy_file, host=None)
...
def test_informer_to_file_with_host(tmp_path):
    ...
    informer.to_file(dummy_file, host="remote_host")

    info_mock.to_file.assert_called_once_with(dummy_file, "remote_host")
...
def test_informer_to_file_no_host(tmp_path):
    ...
    dummy_file = tmp_path / "output.qqinfo"
    informer.to_file(dummy_file, host=None)

    info_mock.to_file.assert_called_once_with(dummy_file, None)
...
def test_informer_set_running():
    ...
    informer.set_running(time_val, main_node, all_nodes, work_dir)
...
def test_informer_set_finished():
    ...
    informer.set_finished(time_val)
...
def test_informer_set_failed():
    ...
    time_val = datetime(2025, 1, 1, 15, 0)
    exit_code = 42
    informer.set_failed(time_val, exit_code)
...
def test_informer_uses_scratch_returns_value():
    ...
    resources_mock.uses_scratch.return_value = True

    info_mock = MagicMock(spec=Info)
    info_mock.resources = resources_mock

    informer = Informer(info_mock)
    assert informer.uses_scratch() is True

    resources_mock.uses_scratch.return_value = False
    assert informer.uses_scratch() is False
...
def test_informer_get_destination_returns_none_when_missing():
    ...
    informer = Informer(info_mock)
    assert informer.get_destination() is None
...
def test_informer_get_batch_state_no_cache():
    ...
    batch_job_info_mock.get_state.return_value = BatchState.RUNNING

    batch_system_mock = MagicMock()
    batch_system_mock.get_batch_job.return_value = batch_job_info_mock
    ...
    state = informer.get_batch_state()

    batch_system_mock.get_batch_job.assert_called_once_with("12345")
    batch_job_info_mock.get_state.assert_called_once()
    assert state == BatchState.RUNNING


def test_informer_get_batch_state_with_cache():
    batch_job_info_mock = MagicMock()
    batch_job_info_mock.get_state.return_value = BatchState.FINISHED
    ...
    batch_system_mock.get_batch_job.assert_not_called()
    batch_job_info_mock.get_state.assert_called_once()
...
def test_informer_get_real_state():
    ...
    with (
        patch.object(
            informer, "get_batch_state", return_value=BatchState.RUNNING
        ) as batch_state_patch,
        patch(
            "qq_lib.info.informer.RealState.from_states", return_value=RealState.BOOTING
        ) as mock_from_states,
    ):
...
def test_informer_get_comment_no_cache():
    batch_job_info_mock = MagicMock()
    batch_job_info_mock.get_comment.return_value = "Job comment"
    ...
    comment = informer.get_comment()

    batch_system_mock.get_batch_job.assert_called_once_with("12345")
    batch_job_info_mock.get_comment.assert_called_once()
    assert comment == "Job comment"


def test_informer_get_comment_with_cache():
    batch_job_info_mock = MagicMock()
    batch_job_info_mock.get_comment.return_value = "Cached comment"
...
def test_informer_get_estimated_no_cache():
    ...
    batch_job_info_mock = MagicMock()
    batch_job_info_mock.get_estimated.return_value = estimated_mock
    ...
    result = informer.get_estimated()

    batch_system_mock.get_batch_job.assert_called_once_with("12345")
    batch_job_info_mock.get_estimated.assert_called_once()
...
def test_informer_get_main_node_no_cache():
    batch_job_info_mock = MagicMock()
    batch_job_info_mock.get_main_node.return_value = "node1"
    ...
    result = informer.get_main_node()

    batch_system_mock.get_batch_job.assert_called_once_with("12345")
    batch_job_info_mock.get_main_node.assert_called_once()
    assert result == "node1"
...
def test_informer_get_nodes_no_cache():
    batch_job_info_mock = MagicMock()
    batch_job_info_mock.get_nodes.return_value = ["node1", "node2"]
    ...
    result = informer.get_nodes()

    batch_system_mock.get_batch_job.assert_called_once_with("12345")
    batch_job_info_mock.get_nodes.assert_called_once()
...
def test_informer_get_batch_info_fetches_when_not_cached():
    ...
    batch_system.get_batch_job.return_value = batch_job
    ...
    result = informer.get_batch_info()

    assert result is batch_job
    batch_system.get_batch_job.assert_called_once_with("123")
...
def test_informer_from_job_id_returns_informer_when_valid():
    ...
    with (
        ...
        patch(
            "qq_lib.info.informer.Informer.from_batch_job",
            return_value=informer_instance,
        ),
    ):
        result = Informer.from_job_id("123")
...
def test_informer_from_batch_job_raises_on_mismatch():
    ...
    informer_mock.matches_job.return_value = False
...
def test_informer_from_batch_job_returns_informer_on_success():
    ...
    informer_mock = MagicMock()
    informer_mock.matches_job.return_value = True

    with patch("qq_lib.info.informer.Informer.from_file", return_value=informer_mock):
        result = Informer.from_batch_job(batch_job)
...
def test_informer_should_transfer_files_returns_true_on_success():
    ...
    informer = Informer(info_mock)
    result = informer.should_transfer_files(0)
...
def test_informer_should_transfer_files_returns_false_on_success_with_nonzero():
    ...
    informer = Informer(info_mock)
    result = informer.should_transfer_files(1)
...
def test_informer_should_transfer_files_with_always_mode():
    ...
    informer = Informer(info_mock)
    assert informer.should_transfer_files(0) is True
    assert informer.should_transfer_files(1) is True
    assert informer.should_transfer_files(42) is True
...
def test_informer_should_transfer_files_with_never_mode():
    ...
    informer = Informer(info_mock)
    assert informer.should_transfer_files(0) is False
    assert informer.should_transfer_files(1) is False
    assert informer.should_transfer_files(42) is False
...
def test_informer_should_transfer_files_with_transfer_modes_list():
    ...
    assert informer.should_transfer_files(0) is True
    assert informer.should_transfer_files(1) is True
    assert informer.should_transfer_files(2) is False
...
def test_informer_should_archive_files_returns_false_when_not_loop_job():
    ...
    informer = Informer(info_mock)
    result = informer.should_archive_files(0)
...
def test_informer_should_archive_files_returns_false_when_loop_job_fails():
    ...
    informer = Informer(info_mock)
    result = informer.should_archive_files(1)
...
def test_informer_should_archive_files_with_always_mode():
    ...
    informer = Informer(info_mock)
    assert informer.should_archive_files(0) is True
...
def test_informer_should_archive_files_with_never_mode():
    ...
    assert informer.should_archive_files(1) is False
...
def test_informer_should_archive_files_with_exit_code_mode():
    ...
    informer = Informer(info_mock)
    assert informer.should_archive_files(5) is True
    assert informer.should_archive_files(1) is False
    assert informer.should_archive_files(0) is False
