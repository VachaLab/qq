...
class Runner:
    ...
    def _resubmit(self) -> None:
        ...
        if not self._should_resubmit:
            logger.info(
                f"The script finished with an exit code of '{CFG.exit_codes.qq_run_no_resubmit}' "
                f"indicating that the next cycle of the job should not be submitted. Not resubmitting."
            )
            return
